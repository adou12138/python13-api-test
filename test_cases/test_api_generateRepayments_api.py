...
import logger
logger = logger.get_logger(logger_name='generateRepayments_api')
...
class GenerateRepaymentsApiTest(unittest.TestCase):
    ...
    cases_generateRepayments = do_excel.read_excel("generateRepayments_api")  # 读取generateRepayments
    ...
    def test_generateRepayments(self, case):  # 测试注册
        ...
        generateRepayments_api_data_new = Context.replace_new(case.data)  # 调用类的方法替换参数
        resp = self.request.request(case.method, case.url, generateRepayments_api_data_new)

        try:
            self.assertEqual(json.loads(case.expected)['msg'], json.loads(resp.text)['msg'])
            self.do_excel.write_excel('generateRepayments_api', case.case_id + 1, resp.text, 'PASS')  # 读取sheet，写入结果
            logger.info("第{0}用例执行结果：PASS".format(case.case_id))

        except AssertionError as e:
            self.do_excel.write_excel('generateRepayments_api', case.case_id + 1, resp.text, 'FAIL')
